...
export default function CurrProjects({ currProjects, theme, setTheme }) {
  ...
  const trail = useTrail(currProjects.length, {
    opacity: mounted ? 1 : 0,
...
export async function getServerSideProps() {
  try {
    ...
    console.error('[getServerSideProps] /current-projects error:', e);
    return {
      props: { currProjects: [] }
    };
  }
